...
# <changes: ck3255>
def ed_calc(x: torch.Tensor, attention_mask: torch.Tensor, metric: str = "hamming_L1") -> torch.Tensor:
    """
...
def energy_distance(
    ...
    if metric == "L2":
        ...
        # binarise once (bool is fine on recent PyTorch/CUDA)
        xb_bool = (x > 0)                       # [Q, S, E]  bool
        yb_bool = (y > 0)                       # [B, E]     bool

        # broadcast: [Q,S,1,E] XOR [1,1,B,E] → [Q,S,B,E]
        xor = xb_bool.unsqueeze(2) ^ yb_bool.unsqueeze(0).unsqueeze(0)

        # popcount along the last dim → [Q,S,B], cast to float
        dist = xor.sum(-1, dtype=torch.int32).float()

    elif metric == "hamming_quantile":
        # 1 · compute median **only over x** (same as verifier)
        thresh = x.flatten().quantile(0.5)

        # 2 · binarise
        xb_bool = x > thresh            # [Q, S, E]  bool
        yb_bool = y > thresh            # [B, E]     bool

        # 3 · xor + popcount
        xor  = xb_bool.unsqueeze(2) ^ yb_bool.unsqueeze(0).unsqueeze(0)
        dist = xor.sum(-1, dtype=torch.int32).float()     # [Q, S, B]
...
def energy_distance_batched(
    ...
    """
    Compute energy distance between multivector queries and documents in batches to avoid OOM.
    Returns a [Q, D] matrix.
    """
    Q = x.size(0)
    D = y.size(0)
    device = x.device
    ...
    for start in range(0, D, batch_size):
        ...
        y_chunk = y[start:end].to(device)  # [batch_size, E]
        score_chunk = energy_distance(x, y_chunk, attention_mask, metric=metric)  # [Q, batch_size]
        all_scores.append(score_chunk)

    return torch.cat(all_scores, dim=1)  # [Q, D]
